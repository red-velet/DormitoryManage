...
<head>
    ...
    <style>
        ...
        .login-main .login-bottom .center .item .icon-1 {
            background: url('static/image/icon-login.png') no-repeat 1px 0;
        }

        .login-main .login-bottom .center .item .icon-2 {
            background: url('static/image/icon-login.png') no-repeat -54px 0;
        }

        .login-main .login-bottom .center .item .icon-3 {
            background: url('static/image/icon-login.png') no-repeat -106px 0;
        }

        .login-main .login-bottom .center .item .icon-4 {
            background: url('static/image/icon-login.png') no-repeat 0 -43px;
            position: absolute;
            right: -10px;
            cursor: pointer;
        }
        ...
        .login-main .login-bottom .center .item .icon-5 {
            background: url('static/image/icon-login.png') no-repeat -55px -43px;
        }

        .login-main .login-bottom .center .item .icon-6 {
            background: url('static/image/icon-login.png') no-repeat 0 -93px;
            position: absolute;
            right: -10px;
            margin-top: 8px;
            cursor: pointer;
        ...
        .login-main .login-bottom .tip .icon-check {
            ...
            height: 14px;
            border: none;
            background: url('static/image/icon-login.png') no-repeat -111px -48px;
        }
        ...
        body {
            background: url('static/image/bg.jpg') 0% 0% / cover no-repeat;
            position: static;
            font-size: 12px;
        }
...
<div class="main-body">
    <div class="login-main">
        <div class="login-top">
            <span>学生宿舍管理系统后台登录</span>
            <span class="bg1"></span>
            <span class="bg2"></span>
        </div>
        <form class="layui-form layui-form-pane login-bottom">
            <div class="center">
                <div class="item">
            ...
            <div class="layui-form-item" style="text-align:center; width:100%;height:100%;margin:0px;">
                <button class="login-btn" type="button" lay-submit="" lay-filter="login">立即登录</button>
            </div>
        </form>
    </div>
</div>
...
<script>
    layui.use(['form', 'jquery'], function () {
        ...
        {
            $.post('user/login',{"username":username,"password":password},function(reply){
                console.log(reply)
                if(reply.message === "success")
                {
                    ...
                    {
                        case "0":
                            //角色为超级管理员,跳转到超级管理员的主页
                            location.href = "user/admin/main";
                            break;
                        case "1":
                            //角色为宿舍管理员,跳转到宿舍管理员的主页
                            location.href = "user/dormManager/main";
                            break;
                        case "2":
                            //角色为学生,跳转到学生的主页
                            location.href = "user/student/main";
                            break;
                        default:
                            layer.msg("角色选中错误");
                            break
